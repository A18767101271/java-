(function (win, lib) {
    ...
    function apiRequestCore(api, ver, data, success, error) {

        var jd = data || {};
        jd.api = api;
        jd.ver = ver;
        jd.app_id = options.appId;
        jd.timestamp = util.getTimestamp();
        jd.sign = util.md5(getSignString(jd)).toUpperCase();
    ...
    function apiRequest(opt) {
        ...
        apiRequestCore(api, ver, data, function (data) {
    ...
    function getUserInfo(callback) {
        if (callback && typeof callback === 'function') {
            apiRequestCore('kk.h5.account.simpleinfo.get', '1.0', {}, function (data) {
                var user = null;
    ...
    function getUserNick(callback) {
        ...
        if (callback && typeof callback === 'function') {
            ...
            var nick = util.getCookie("nickname");
            return nick;
        }
    }

    function generateLoginUrl() {
        return options.loginUrl + "?appid=" + encodeURIComponent(options.appId) + "&redirect=" + encodeURIComponent(window.location);
    }
    var isGoingToLogin = false;
