...
public final class JsonUtil {
    ...
    public static String toJson(final Object o) {
        ...
        } else if (o instanceof Object[]) // 对象数组
        {
            return array2Json((Object[]) o);
        } else if (o instanceof int[])// 基本类型数组
        {
            return intArray2Json((int[]) o);
        } else if (o instanceof boolean[])// 基本类型数组
        {
            return booleanArray2Json((boolean[]) o);
        } else if (o instanceof long[])// 基本类型数组
        {
            return longArray2Json((long[]) o);
        } else if (o instanceof float[])// 基本类型数组
        {
            return floatArray2Json((float[]) o);
        } else if (o instanceof double[])// 基本类型数组
        {
            return doubleArray2Json((double[]) o);
        } else if (o instanceof short[])// 基本类型数组
        {
            return shortArray2Json((short[]) o);
        } else if (o instanceof byte[])// 基本类型数组
        {
            return byteArray2Json((byte[]) o);
        } else if (o instanceof Number) // Number
        {
            return number2Json((Number) o);
        } else if (o instanceof Object) // 保底收尾对象
        {
     ...
     * @return JSON格式
     */
    static String bigDecimal2Json(final BigDecimal number) {
        return number.toString();
    }
    ...
    public static String object2Json(final Object bean) {
        ...
        for (final Method method : methods) {
            try {
                ...
                if ("getLastYearSaleAmt".equals(name)) {
                ...
                if (key.length() > 0 && Character.isUpperCase(key.charAt(0))
                    ...
                    // elementObj.toString());
                    /**
                     * 请求的key属性值非空检查，当为空对象或空字符串时，不进行请求上送处理，避免后台收到 的报文字段出现无法解析的情况
                     *
                     * Modify by chumk at 2016-05-13
                     */
                    if (elementObj == null) {
                        continue;
                    }
                    boolean objectIsEmpty = isEmpty(elementObj.toString());
                    if (!objectIsEmpty) {
                        sb.append('\"');
                        sb.append(key); // 不能包含特殊字符
                        sb.append('\"');
    ...
    public static void main(String[] args) {
        System.err.println(bigDecimal2Json(new BigDecimal("900000")));
    }
}
